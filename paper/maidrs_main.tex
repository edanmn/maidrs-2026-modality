...
\title{How We Nearly Wrote Off a Modality: Model-Class Mismatch, Aggregation, and What Survived Replication in Structure--Transcriptome Drug Repurposing}
...
also loses to four stronger structural baselines. The more useful result is how
...
$0.548$ on Phase~2, 95\% CI $[0.518, 0.577]$). Two ordinary choices produced that
...
resulting dense features. Correcting both reaches $0.748$ $[0.723, 0.773]$.
...
random forest by $+0.09$ to $+0.14$ in seven of eight release-by-representation
...
specified expression arm stays redundant with structure (blend gain $+0.002$,
$p{=}0.31$), replicated on SIDER side-effect prediction, while gene-level analysis
...
\begin{enumerate}
    ...
    baselines (Sections~\ref{sec:f1} and~\ref{sec:strong}).
    ...
    the expression modality at $0.548$ on Phase~2 (95\% CI $[0.518, 0.577]$).
    Correcting both reached $0.748$ $[0.723, 0.773]$ (Section~\ref{sec:f2}).

    \item \textbf{Only half of that explanation replicates.} On LINCS Phase~1
    the model-class effect holds at $+0.09$ to $+0.14$ across representations,
    ...
    structure alone ($+0.002$, $p{=}0.31$), replicated on a second task family
...
\else
\textbf{Our own prior work in this line.} We previously applied deep learning to
epilepsy drug repurposing on LINCS data~\cite{kondadadi2025ctdn}, using a
temporal-diffusion architecture and reporting favourable results against
...
Multi-representation gradient boosting & 0.582 \\
\midrule
...
default settings is the one poor performer ($0.582$), which we attribute to
...
no drug-level signal: expression-only sits at $0.506$, indistinguishable from the
random control. We now believe that reading was wrong, and that the path to it is
...
$0.548$ to $0.640$ with the model held fixed at RF (Table~\ref{tab:corners});
the 6-repeat grid in Figure~\ref{fig:grid} puts the same shift at $0.525$ to
$0.641$. Figure~\ref{fig:cancel} shows the cancellation directly.
...
the \emph{same} naively aggregated features reaches $0.663$ $[0.636, 0.688]$
where the random forest reaches $0.548$ $[0.518, 0.577]$. Structure, by contrast,
...
(high-dose aggregation $+$ logistic regression) reaches $0.748$ $[0.723, 0.773]$.
...
space, so their means for the same cells differ by up to $0.015$ and individual
cells there carry wider uncertainty.
...
\midrule
Naive mean, all conditions & RF & 0.548 & $[0.518, 0.577]$ \\
Naive mean, all conditions & LogReg & 0.663 & $[0.636, 0.688]$ \\
High-dose 10\,\textmu M / 24\,h & RF & 0.640 & $[0.612, 0.668]$ \\
High-dose 10\,\textmu M / 24\,h & LogReg & \textbf{0.748} & $[0.723, 0.773]$ \\
\bottomrule
\end{tabular}
\end{table}
...
concrete demonstration of how a reported chance-level baseline can be
manufactured by preprocessing and model-selection defaults rather than by an
absence of signal.
...
between-model variance accounts for $68.3\%$ of total variance versus $12.5\%$
for between-aggregation, a ratio of $0.2\times$. We report this because we
...
Fitting an $L_2$-regularised logistic model per class on the corrected
landmark features and ranking genes by differential response between class
members and the rest, the three strongest genes for statins are HMGCS1
($\Delta = +3.66$, $p = 8.2 \times 10^{-4}$), RHOA
($+3.21$, $p = 8.5 \times 10^{-4}$) and HMGCR
($+2.62$, $p = 6.2 \times 10^{-4}$). HMGCR encodes
...
For SSRIs the leading genes are EBP ($+1.71$), TMEM97 ($+1.69$), FDFT1
($+2.16$) and HMGCS1 ($+3.08$), all sterol-pathway members. TMEM97 is the
sigma-2 receptor and EBP the sigma-1-associated sterol isomerase, both
established off-target binding partners of cationic amphiphilic drugs, a class
to which most SSRIs belong.
...
HMGCR sits at $+0.93$ under statins against $+0.18$ elsewhere.
...
\midrule
Naive mean, all conditions & 0.525 & 0.664 & 0.586 & \textbf{0.719} \\
High-dose 10\,\textmu M / 24\,h & 0.641 & \textbf{0.733} & 0.559 & 0.697 \\
Strongest single signature & 0.583 & 0.714 & 0.528 & 0.492 \\
Gold / exemplar signature mean & 0.587 & 0.727 & 0.559 & 0.681 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{The model-class effect replicates.} Logistic regression beats random
forest by $+0.09$ to $+0.14$ in seven of the eight release-by-representation
...
dense high-dimensional expression features costs roughly $0.13$ AUROC, and it
costs it consistently.

\textbf{The aggregation effect does not replicate, and reverses.} On Phase~2,
restricting to high-dose 24-hour signatures improved matters by $+0.12$ (RF) and
$+0.07$ (LogReg). On Phase~1 the same restriction \emph{hurts}, by $-0.03$ and
$-0.02$, and naive averaging over all conditions is the best representation
...
\midrule
Mean AUROC (pooled, $n{=}120$) & 0.9160 & 0.7468 & 0.9178 \\
\bottomrule
\end{tabular}
\end{center}

a gain of $+0.0018$ ($p = 0.31$, $27/120$ wins). Expression carries real signal
($0.747 \gg$ chance) but that signal is largely subsumed by structure for this
task. Per-class gains range from $-0.004$ to $+0.011$ and are individually
non-significant.

\textbf{Replication on a second task family.} Pharmacological class is arguably
...
$60$ side effects, structure reaches $0.601$, expression $0.530$, and the blend
$0.583$; blending \emph{hurts} ($-0.018$, $p = 1.9\times10^{-7}$).
Restricted to transcriptionally responsive drugs and using gold-signature
per-cell-line features (the closest replication of~\cite{wang2016} we could
construct from Phase~2 data) gives structure $0.588$, expression $0.544$, blend
$0.589$ ($+0.001$, $p = 0.99$). \textbf{We did not replicate the published
result}, and we flag this as an open discrepancy rather than a refutation: we
lack the Phase~1 QC fields and the GO-enrichment transform used in that work.
...
\section{Self-audit: four defects, each of which produced a publishable result}
\label{sec:audit}

Over the course of this study we found four defects in our own pipeline. We list
...
\textbf{We did not evaluate published contrastive methods.} Our contrastive
model is our own implementation of a standard CLIP-style aligner, and the
baselines it loses to are also ours. We did not run the published
drug-disease contrastive methods that this literature has produced. Our claim is
therefore that \emph{a} reasonable contrastive implementation loses to a random
forest under this protocol, and not that every published method would. We regard
this as the most substantial gap in the paper.
...
All code, result logs and figure-regeneration scripts are at
\url{https://github.com/edanmn/maidrs-2026-modality}. Each figure script prints
the values this paper quotes from it, so every number can be checked against a
fresh run rather than read off the figure, and the repository maps each table
and claim to the log it came from. The raw LINCS releases (GSE70138, GSE92742)
...
\bibitem{kondadadi2025ctdn} R. Kondadadi, ``Causal temporal diffusion networks for drug repurposing in epilepsy,'' \emph{Briefings in Bioinformatics}, vol. 26, suppl. 1, p. i42, 2025, conference abstract.
