...
\title{Model-Class Mismatch and Aggregation Hide Transcriptomic Signal in LINCS Drug Repurposing}
...
\IEEEauthorblockA{\textit{Eastview High School}, \textit{University of Minnesota} \\
Apple Valley, MN, USA \\
konda052@umn.edu}
\and
...
\begin{abstract}
A near-chance baseline is usually read as evidence that a modality carries no
signal. For LINCS L1000 transcriptomics it is more often evidence about the
pipeline. Averaged across all doses, cell lines and timepoints and fed to a
random forest, drug-level expression recovers pharmacological class at AUROC
$0.548$ on Phase~2 (95\% CI $[0.520, 0.577]$), close enough to chance to read as
absence. That averaging cancels $64\%$ of signature magnitude, and tree
ensembles suit dense high-dimensional features poorly. With both corrected,
expression reaches $0.752$ $[0.728, 0.776]$, and the corrected features recover
known pharmacology unsupervised, placing HMGCR, the direct target of every
statin, among the top ten of $978$ landmark genes ($q \le 0.018$). An independent
release (LINCS Phase~1, 46{,}726 signatures) separates the general half of this
diagnosis from the dataset-specific half. Model-class mismatch costs roughly
$0.14$ AUROC in seven of eight release-by-representation combinations, while the
aggregation effect reverses sign between releases and has to be settled per
dataset. All of this came out of a negative control that failed decisively. A
CLIP-style structure-expression aligner lost to a random forest on ECFP4
fingerprints in every class ($p \le 0.027$), as did four stronger structural
baselines and the published alignment objective, at $0.751$ against $0.917$
($p \le 0.0039$). Correctly specified, expression is still redundant with
structure for these tasks ($+0.001$, $p{=}0.15$). A self-audit reports five
defects in our own pipeline, each of which produced a publishable-looking but
incorrect number.
\end{abstract}

\begin{IEEEkeywords}
drug repurposing, multi-modal learning, LINCS L1000, transcriptomics, reproducibility
\end{IEEEkeywords}
...
Our starting point was a negative control for that recipe, and it failed. The
diagnosis we first gave for the failure was wrong, and correcting it recovered a
modality we had been ready to write off. Our contributions:

\begin{enumerate}
    \item A modality that looks uninformative is recoverable. Measured
    the way most pipelines measure it, drug-level L1000 expression sits at
    $0.548$ on Phase~2 (95\% CI $[0.520, 0.577]$), close enough to chance to
    read as absence of signal. Two measurement defaults produce that number.
    With both corrected it reaches $0.752$ $[0.728, 0.776]$ (Section~\ref{sec:f2}),
    and the corrected features recover the canonical statin target without
    supervision (Section~\ref{sec:bio}).

    \item One half of the diagnosis is a general hazard, the other is
    dataset-specific. On LINCS Phase~1 the model-class effect holds at $+0.09$
    to $+0.16$ across representations and costs roughly $0.14$ AUROC, while the
    aggregation effect reverses sign. Practitioners should choose the model class
    per modality and determine aggregation on their own data
    (Section~\ref{sec:repl}).

    \item The negative control holds against the published
    objective. Across six real pharmacological classes under a
    leak-free, drug-disjoint protocol, a random forest on raw ECFP4 fingerprints
    outperforms every contrastive variant we trained, in every class, and so do
    four stronger structural baselines. The published alignment objective of
    ...
    \item Correctly specified, the second modality is still redundant
    here. With the expression arm properly constructed, blending does not
    improve on structure alone ($+0.001$, $p{=}0.15$), and the same holds on a
    second task family (Section~\ref{sec:f3}).
\end{enumerate}

Section~\ref{sec:audit} lists five defects we found in our own pipeline. Each
produced a result that looked publishable, and two of them show a failure mode
that is hard to see from outside: a chain of analyses that look like independent
confirmations while all consuming the same defective input.

\section{Related work}

Finlayson et al.~\cite{finlayson2021} introduced the pairing of chemical
structure with perturbational expression under a joint contrastive objective, and
also flagged generalisation across cell
...
Our diagnosis rests on findings that
...
\ifanon
Deep learning has been applied to epilepsy drug
...
problematic, and it has not been re-evaluated under the corrected pipeline. The
failure mode described here applies to that work as much as to the method we set
out to test.
\else
One of us previously applied deep
...
corrected pipeline. The failure mode described here applies to that earlier work
as much as to the method we set out to test.
\fi

\section{Setup}
\label{sec:setup}

We use LINCS L1000 Phase~2 (GSE70138) Level-5 signatures over $978$
...
Labels are six pharmacologically standard, mutually exclusive drug classes
...
Evaluation uses repeated Monte-Carlo drug-disjoint subsampling: for each
...
\section{The negative control: structure alone beats cross-modal alignment}
\label{sec:f1}
...
features fed jointly to one model) is worse than structure alone in all
...
(Concat+RF) is worse than structure alone in all six classes.}
\label{fig:methods}
\end{figure*}
...
Our contrastive model loses to every structural method we tried, including the
kernel formulation that this literature treats as standard.
...
Mean AUROC over the six classes is $0.751$ against $0.715$ for our CLIP-style
model, so the published objective is the stronger of the two contrastive
methods. A random forest on ECFP4 beats it in all six
...
We depart from the published description in two places. The
...
\section{Two measurement defaults suppress the expression modality}
\label{sec:f2}

Our first reading of Table~\ref{tab:main} was that L1000 expression carries no
drug-level signal, since expression-only averages $0.507$ over the six classes,
indistinguishable from the random control. That reading was wrong.

To obtain one feature vector per drug we had averaged all of
a compound's Level-5 signatures across doses, cell lines, and timepoints, a
common default that cancels signal. The median norm of a drug's averaged
signature is $11.3$ against $31.3$ for its individual signatures, so averaging
removes $64\%$ of signature magnitude. Individual signatures carry plenty of
signal, since $99.9\%$ of compounds have at least one strongly active signature ($\ge 50$
...
Independently, we had used a random forest for the
...
the same naively aggregated features reaches $0.663$ $[0.637, 0.689]$
...
Either choice alone leaves substantial signal; both
...
repeats per cell and are intended to show the pattern across the design
...
This grid shows the pattern; the four corner cells are re-estimated at 30
...
This is probably not unique to us. Our naive-conjunction estimate
($0.548$) and published raw-signature baselines (median AUROC $0.507$ for
TS-978 and $0.485$ for TS-12328 in the MoAble MoA benchmark~\cite{jang2021})
both sit close to chance. This is
agreement in range only. Our interval does not contain $0.507$, so
...
differential-expression strength~\cite{lin2021}. Our result is a
...
Given the above, we hypothesised
...
for between-aggregation, a ratio of $0.16\times$. We had intended this as the
paper's headline.
...
pipeline might be overfitting a preprocessing choice to a metric. Features that
carry real biology should recover known drug mechanisms without supervision, so
we checked for that.
...
We ran this analysis to characterise the features, not to look for particular
genes, and it returned the canonical target of one class and a documented
off-target pathway of another. It is a positive control on the corrected
pipeline and the clearest evidence we have that the naive pipeline's
chance-level result hid real biological signal.
...
The model-class effect replicates. Logistic regression beats random
...
weakest representation overall. Across both releases, a tree ensemble on dense
high-dimensional expression features costs roughly $0.14$ AUROC.

The aggregation effect does not replicate, and reverses. On Phase~2,
restricting to high-dose 24-hour signatures improved matters by $+0.09$ under
both model classes. On Phase~1 the same restriction hurts, by $-0.04$
...
Model-class mismatch is a hazard for this modality in both
releases. Aggregation matters but depends on the release, and our chance-level
Phase~2 number required both effects. A practitioner should not take from this
``always restrict to high dose''; which part of the pipeline is responsible has
to be tested on their own data.

\section{A correctly specified expression arm stays redundant with structure}
\label{sec:f3}

We next asked whether a properly specified expression arm is complementary to
structure. With the best model chosen per modality
...
Pharmacological class is largely
...
$0.573$; blending hurts ($-0.017$, $p = 1.8\times10^{-4}$).
...
expression ahead on only $87$ of the $546$ endpoints). We did not replicate the
published result on either release, but we lack the Phase~1 QC filtering and the
GO-enrichment transform used in that work, so we treat this as an open
discrepancy.

On transcriptionally responsive compounds (strongest signature at
$\mathrm{TAS} \ge 0.3$, following Sirci et al.~\cite{sirci2017}) with
gold-signature per-cell-line features, the fixed blend does gain $+0.021$ over structure on drug-class recovery
($p = 0.017$, 40 repeats). This cuts against our conclusion, and we do not
read much into it. The same cell with a random-forest expression
...
An earlier version of this analysis appeared to show a $+0.008$ gain. It came
entirely from selecting the blend weight on test predictions and vanished under
nested selection.

\section{Self-audit: five defects, each of which produced a publishable result}
\label{sec:audit}

We found five defects in our own pipeline over the course of this study. Each
yielded a plausible, presentable result, and the interaction between two of them
is the main methodological warning in this paper.

\begin{enumerate}
    \item Zero-vector embeddings. In the class-holdout regime, drug-level
    ...
    \item Single-seed regime comparison. The full and class-holdout
    ...
    \item Naive signature aggregation (Section~\ref{sec:f2}).

    \item Mismatched model class for the expression arm
    (Section~\ref{sec:f2}).

    \item Zero-filled representations, found while preparing the code
    ...
    the corrected fallback, and no conclusion changed.
\end{enumerate}

Defects 3 and 4 compounded. Once defect~3 had convinced us that the
...
all consumed the same defective features. Mutual confirmation among analyses
...
result remains an open counterexample.

\section{Conclusion}

On LINCS L1000, a near-chance expression baseline turned out to be a property of
the pipeline. Drug-level
expression sits at $0.548$ under two ordinary measurement defaults, averaging
Level-5 signatures over every experimental condition and reusing the structure
arm's model class, and at $0.752$ once both are corrected. The corrected
features put HMGCR, the direct molecular target of every statin, in the top ten of
$978$ landmark genes at $q \le 0.018$, alongside HMGCS1 and RHOA.

Half of that diagnosis generalises. On an independent LINCS release, model-class
mismatch costs roughly $0.14$ AUROC in seven of eight release-by-representation
combinations. The aggregation effect reverses, with the restriction that helped
on Phase~2 hurting on Phase~1, where naive averaging is the best representation
we tried. We wrote the conjunction claim before running that replication, and
half of it did not hold. Model class should be chosen per modality everywhere,
while aggregation has to be settled on the data at hand.

In the negative control, a CLIP-style aligner lost to a random forest on
fingerprints in all six disease areas, as did the published objective this recipe
comes from once reimplemented and tuned under the same protocol, and four
stronger structural baselines beat both. Correctly specified, the expression arm
carries real signal but remains redundant with structure for these tasks; making
it complementary is a problem this paper leaves open.
...
Results come from LINCS L1000 Phase~2 with replication on
...
Class sizes of 5--16 drugs bound per-class
...
\bibitem{jang2021} G. Jang, S. Park, S. Lee, S. Kim, S. Park, and J. Kang, ``Predicting mechanism of action of novel compounds using compound structure and transcriptomic signature coembedding,'' \emph{Bioinformatics}, vol. 37, suppl. 1, pp. i376--i382, Jul. 2021, doi: 10.1093/bioinformatics/btab275.
